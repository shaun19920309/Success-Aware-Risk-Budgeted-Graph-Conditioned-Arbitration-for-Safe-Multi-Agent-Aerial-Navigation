...
def get_task_name(env, env_args):
    ...
    if env == "smac":
        ...
        obstacle = "obstacle" if env_args.get("use_obstacles", False) else "no_obstacle"
        task = f"{env_args.get('quads_mode', 'static_same_goal')}_{env_args.get('num_agents', 4)}agents_{obstacle}"
    return task
