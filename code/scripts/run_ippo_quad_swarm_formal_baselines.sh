...
run_case() {
  ...
  echo "=== IPPO formal case: ${result_name} ==="
  QUADS_MODE="$quads_mode" \
  NUM_AGENTS="$num_agents" \
  USE_OBSTACLES="$use_obstacles" \
  TRAIN_DIR="$BASE/results/ippo_quad_swarm/$result_name" \
  ...
  bash "$BASE/scripts/run_ippo_quad_swarm.sh"
}
...
echo "IPPO formal QuadSwarm baseline run complete."
