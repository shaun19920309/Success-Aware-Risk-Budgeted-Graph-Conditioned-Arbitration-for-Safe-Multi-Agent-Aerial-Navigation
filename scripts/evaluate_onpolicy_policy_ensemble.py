...
def predicted_team_action_metrics(
    env: QuadSwarmOnPolicyEnv,
    actions: np.ndarray,
    horizon: float,
) -> tuple[float, float, float]:
    """Approximate one-step pair clearance, obstacle clearance, and goal distance."""
    ...
    if (
        pos.ndim != 2
        or actions.ndim != 2
        or len(pos) != len(actions)
        or actions.shape[1] < 3
    ):
        return math.nan, math.nan, math.nan

    next_pos = pos + max(float(horizon), 0.0) * actions[:, :3]
    next_pair = min_pair_distance_from_pos(next_pos)
    next_obstacles = obstacle_clearance_for_positions(env, next_pos)
    next_obstacle = (
        float(np.nanmin(next_obstacles))
        if next_obstacles.size and np.any(np.isfinite(next_obstacles))
        else math.inf
    )
    goals = swarm_goals(env)
    next_goal = (
        float(np.mean(np.linalg.norm(next_pos - goals, axis=1)))
        if goals is not None and len(goals) == len(next_pos)
        else math.nan
    )
    return next_pair, next_obstacle, next_goal


def pairwise_kinematic_risk_metrics(
    env: QuadSwarmOnPolicyEnv,
    lookahead_seconds: float,
) -> tuple[float, float, float]:
    """Return current distance, finite-horizon CPA distance, and closing speed.

    The closest-point-of-approach (CPA) calculation uses the simulator's
    current positions and velocities. Positive closing speed means at least one
    pair is approaching. The three team-level scalars keep the intervention
    rule independent of expert logits and avoid blending policy actions.
    """

    pos, vel = swarm_pos_vel(env)
    if pos.ndim != 2 or vel.shape != pos.shape or len(pos) < 2:
        return math.inf, math.inf, 0.0

    horizon = max(float(lookahead_seconds), 0.0)
    min_distance = math.inf
    min_cpa_distance = math.inf
    max_closing_speed = 0.0
    eps = 1e-8
    for i in range(len(pos)):
        for j in range(i + 1, len(pos)):
            relative_pos = np.asarray(pos[i] - pos[j], dtype=np.float64)
            relative_vel = np.asarray(vel[i] - vel[j], dtype=np.float64)
            distance = float(np.linalg.norm(relative_pos))
            if not math.isfinite(distance):
                continue
            min_distance = min(min_distance, distance)
            if distance > eps:
                closing_speed = -float(np.dot(relative_vel, relative_pos / distance))
                max_closing_speed = max(max_closing_speed, closing_speed)

            speed_sq = float(np.dot(relative_vel, relative_vel))
            if speed_sq > eps:
                cpa_time = float(
                    np.clip(-np.dot(relative_pos, relative_vel) / speed_sq, 0.0, horizon)
                )
            else:
                cpa_time = 0.0
            cpa_distance = float(
                np.linalg.norm(relative_pos + relative_vel * cpa_time)
            )
            if math.isfinite(cpa_distance):
                min_cpa_distance = min(min_cpa_distance, cpa_distance)

    return min_distance, min_cpa_distance, max(max_closing_speed, 0.0)


def critical_interaction_pair(
    env: QuadSwarmOnPolicyEnv,
    lookahead_seconds: float,
) -> tuple[tuple[int, int] | None, dict[str, float]]:
    """Identify the pair with the smallest finite-horizon CPA distance.

    The rule uses only positions and velocities available at execution time.
    Current distance breaks CPA ties so collection and runtime select the same
    ...
    for i in range(len(pos)):
        for j in range(i + 1, len(pos)):
            ...
            if speed_sq > eps:
                cpa_time = float(
                    np.clip(
                        -np.dot(relative_pos, relative_vel) / speed_sq,
                        0.0,
                        horizon,
                    )
                )
            ...
            if distance > eps:
                closing_speed = max(
                    -float(np.dot(relative_vel, relative_pos / distance)),
                    0.0,
                )
            if not math.isfinite(cpa_distance):
                continue
            key = (cpa_distance, distance, float(i), float(j))
            if key < best_key:
                best_key = key
                best_pair = (i, j)
                best_metrics = {
...
def critical_interaction_pair_features(
    env: QuadSwarmOnPolicyEnv,
    lookahead_seconds: float,
    *,
    distance_scale: float = 1.4,
) -> tuple[dict[str, np.ndarray], tuple[int, int] | None]:
    """Return per-agent observable features marking the critical graph edge."""

    pair, metrics = critical_interaction_pair(env, lookahead_seconds)
    n_agents = int(getattr(env, "n_agents", 0))
    member = np.zeros(n_agents, dtype=np.float32)
    current_threat = np.zeros(n_agents, dtype=np.float32)
    cpa_threat = np.zeros(n_agents, dtype=np.float32)
    closing = np.zeros(n_agents, dtype=np.float32)
    cpa_time = np.zeros(n_agents, dtype=np.float32)
    if pair is not None:
        indices = np.asarray(pair, dtype=np.int64)
        member[indices] = 1.0
        scale = max(float(distance_scale), 1e-6)
        current_threat[indices] = np.clip(
            (scale - metrics["distance"]) / scale,
            0.0,
            1.0,
        )
        cpa_threat[indices] = np.clip(
            (scale - metrics["cpa_distance"]) / scale,
            0.0,
            1.0,
        )
        closing[indices] = np.clip(metrics["closing_speed"] / scale, 0.0, 2.0)
        horizon = max(float(lookahead_seconds), 1e-6)
        cpa_time[indices] = np.clip(metrics["cpa_time"] / horizon, 0.0, 1.0)
    return (
        {
            "critical_pair_member": member,
            "critical_pair_current_threat": current_threat,
            "critical_pair_cpa_threat": cpa_threat,
            "critical_pair_closing": closing,
            "critical_pair_time": cpa_time,
        },
        pair,
    )


def kinematic_top1_weight(
    env: QuadSwarmOnPolicyEnv,
    gate_context: Dict[str, object],
    params: Dict[str, float],
) -> tuple[float, str]:
    """Select a complete backup policy only for worsening pairwise risk.

    The anchor remains active for close-but-separating pairs. The backup is
    selected when the emergency radius is crossed or when an approaching pair
    is predicted to enter the CPA risk radius within the lookahead horizon.
    A short minimum dwell and a separate exit radius suppress frame-to-frame
    switching without imposing a long safety-policy occupancy.
    """

    enter_distance = max(params.get("t", 1.2), 0.0)
    exit_distance = max(params.get("u", enter_distance), enter_distance)
    emergency_distance = np.clip(params.get("e", 0.65), 0.0, enter_distance)
    cpa_distance_limit = np.clip(
        params.get("q", 0.8),
        emergency_distance,
        enter_distance,
    )
    closing_speed_min = max(params.get("v", 0.05), 0.0)
    lookahead_seconds = max(params.get("h", 0.5), 0.0)
    minimum_dwell = max(int(round(params.get("d", 5.0))), 1)

    current_distance, cpa_distance, closing_speed = pairwise_kinematic_risk_metrics(
        env,
        lookahead_seconds,
    )
    emergency = math.isfinite(current_distance) and current_distance < emergency_distance
    approaching_risk = (
        math.isfinite(current_distance)
        and current_distance < enter_distance
        and closing_speed >= closing_speed_min
        and math.isfinite(cpa_distance)
        and cpa_distance < cpa_distance_limit
    )

    active = bool(gate_context.get("kinematic_top1_active", False))
    dwell_left = max(int(gate_context.get("kinematic_top1_dwell_left", 0)), 0)
    if not active and (emergency or approaching_risk):
        gate_context["kinematic_top1_active"] = True
        gate_context["kinematic_top1_dwell_left"] = minimum_dwell - 1
        state = "emergency" if emergency else "approaching"
        return 1.0, f"kinematic_top1_{state}"

    if active:
        if dwell_left > 0:
            gate_context["kinematic_top1_dwell_left"] = dwell_left - 1
            return 1.0, "kinematic_top1_dwell"
        should_hold = emergency or approaching_risk or (
            math.isfinite(current_distance)
            and current_distance < exit_distance
            and math.isfinite(cpa_distance)
            and cpa_distance < cpa_distance_limit
            and closing_speed > 0.0
        )
        if should_hold:
            return 1.0, "kinematic_top1_hold"
        gate_context["kinematic_top1_active"] = False
        gate_context["kinematic_top1_dwell_left"] = 0
        return 0.0, "kinematic_top1_release"

    return 0.0, "kinematic_top1_anchor"


def decision_point_recovery_weight(
    env: QuadSwarmOnPolicyEnv,
    ...
    """Route to a recovery expert only at safe, progress-stalled decision points.

    Policy A is the default anchor and policy B is a short-lived recovery
    expert. The rule is deliberately team-level and top-1: it never averages
    actions from independently trained controllers.
    """

    window = max(int(round(params.get("w", 20.0))), 2)
    progress_threshold = max(params.get("p", 0.02), 0.0)
    safe_pair = max(params.get("s", 1.2), 0.0)
    safe_obstacle = max(params.get("o", 0.2), 0.0)
    horizon = max(params.get("h", 0.25), 0.0)
    goal_advantage = params.get("g", 0.0)
    safety_tolerance = max(params.get("r", 0.05), 0.0)
    dwell = max(int(round(params.get("d", 20.0))), 1)
    cooldown = max(int(round(params.get("c", 10.0))), 0)

    current_goal = float(metrics.get("mean_goal_dist", math.nan))
    history = list(gate_context.get("decision_point_goal_history", []))
    if math.isfinite(current_goal):
        history.append(current_goal)
    if len(history) > window:
        history = history[-window:]
    gate_context["decision_point_goal_history"] = history
    window_progress = (
        float(history[0] - history[-1]) if len(history) >= window else math.inf
    )
    stalled = len(history) >= window and window_progress <= progress_threshold

    current_pair = float(metrics.get("min_pair_dist", math.nan))
    obstacle_distances = obstacle_clearance(env)
    current_obstacle = (
        float(np.nanmin(obstacle_distances))
        if obstacle_distances.size and np.any(np.isfinite(obstacle_distances))
        else math.inf
    )
    state_is_safe = (
        (not math.isfinite(current_pair) or current_pair >= safe_pair)
        and current_obstacle >= safe_obstacle
    )

    remaining = max(int(gate_context.get("decision_point_recovery_remaining", 0)), 0)
    cooldown_left = max(int(gate_context.get("decision_point_cooldown", 0)), 0)
    if remaining > 0:
        if state_is_safe:
            gate_context["decision_point_recovery_remaining"] = remaining - 1
            if remaining == 1:
                gate_context["decision_point_cooldown"] = cooldown
            return 1.0, "decision_recovery_active"
        gate_context["decision_point_recovery_remaining"] = 0
        gate_context["decision_point_cooldown"] = cooldown
        return 0.0, "decision_anchor_safety_exit"

    if cooldown_left > 0:
        gate_context["decision_point_cooldown"] = cooldown_left - 1
        return 0.0, "decision_anchor_cooldown"

    if not stalled:
        return 0.0, "decision_anchor_progress"
    if not state_is_safe:
        return 0.0, "decision_anchor_stalled_but_unsafe"

    anchor_pair, anchor_obstacle, anchor_goal = predicted_team_action_metrics(
        env,
        anchor_actions,
        horizon,
    )
    recovery_pair, recovery_obstacle, recovery_goal = predicted_team_action_metrics(
        env,
        recovery_actions,
        horizon,
    )
    predicted_progress_advantage = (
        float(anchor_goal - recovery_goal)
        if math.isfinite(anchor_goal) and math.isfinite(recovery_goal)
        else -math.inf
    )
    pair_admissible = (
        not math.isfinite(anchor_pair)
        or not math.isfinite(recovery_pair)
        or recovery_pair + safety_tolerance >= anchor_pair
    )
    obstacle_admissible = (
        not math.isfinite(anchor_obstacle)
        or not math.isfinite(recovery_obstacle)
        or recovery_obstacle + safety_tolerance >= anchor_obstacle
    )
    if (
        predicted_progress_advantage >= goal_advantage
        and pair_admissible
        and obstacle_admissible
    ):
        gate_context["decision_point_recovery_remaining"] = dwell - 1
        return 1.0, "decision_recovery_trigger"
    return 0.0, "decision_anchor_veto"


def decision_point_top1_weight(
    env: QuadSwarmOnPolicyEnv,
    metrics: Dict[str, float],
    anchor_actions: np.ndarray,
    ...
    """Use one complete alternate policy at risk or stalled decision points."""

    risk_enter = max(params.get("t", 1.2), 0.0)
    risk_exit = max(params.get("u", 1.4), risk_enter)
    obstacle_enter = max(params.get("o", 0.2), 0.0)
    horizon = max(params.get("h", 0.25), 0.0)
    safety_advantage_min = params.get("a", 0.0)
    goal_loss_tolerance = max(params.get("l", 0.01), 0.0)
    safety_tolerance = max(params.get("r", 0.05), 0.0)
    window = max(int(round(params.get("w", 20.0))), 2)
    progress_threshold = max(params.get("p", 0.02), 0.0)
    stall_safe_pair = max(params.get("s", risk_exit), risk_enter)
    progress_advantage_min = params.get("g", 0.0)
    recovery_dwell = max(int(round(params.get("d", 20.0))), 1)
    cooldown = max(int(round(params.get("c", 10.0))), 0)

    current_goal = float(metrics.get("mean_goal_dist", math.nan))
    history = list(gate_context.get("decision_top1_goal_history", []))
    if math.isfinite(current_goal):
        history.append(current_goal)
    if len(history) > window:
        history = history[-window:]
    gate_context["decision_top1_goal_history"] = history
    window_progress = (
        float(history[0] - history[-1]) if len(history) >= window else math.inf
    )
    stalled = len(history) >= window and window_progress <= progress_threshold
    ...
    risk_active = bool(gate_context.get("decision_top1_risk_active", False))
    pair_threshold = risk_exit if risk_active else risk_enter
    hazard = (
        (math.isfinite(current_pair) and current_pair < pair_threshold)
        or current_obstacle < obstacle_enter
    )

    recovery_remaining = max(
        int(gate_context.get("decision_top1_recovery_remaining", 0)),
        0,
    )
    cooldown_left = max(int(gate_context.get("decision_top1_cooldown", 0)), 0)
    state_allows_recovery = (
        (not math.isfinite(current_pair) or current_pair >= stall_safe_pair)
        and current_obstacle >= obstacle_enter
    )
    if recovery_remaining > 0:
        if state_allows_recovery:
            gate_context["decision_top1_recovery_remaining"] = recovery_remaining - 1
            if recovery_remaining == 1:
                gate_context["decision_top1_cooldown"] = cooldown
            return 1.0, "decision_top1_recovery_active"
        gate_context["decision_top1_recovery_remaining"] = 0
        gate_context["decision_top1_cooldown"] = cooldown

    need_lookahead = hazard or (stalled and state_allows_recovery and cooldown_left == 0)
    if not need_lookahead:
        gate_context["decision_top1_risk_active"] = False
        if cooldown_left > 0:
            gate_context["decision_top1_cooldown"] = cooldown_left - 1
        return 0.0, "decision_top1_anchor"
    ...
    alternate_pair, alternate_obstacle, alternate_goal = predicted_team_action_metrics(
        env,
        alternate_actions,
        horizon,
    )
    pair_delta = (
        float(alternate_pair - anchor_pair)
        if math.isfinite(anchor_pair) and math.isfinite(alternate_pair)
        else 0.0
    )
    obstacle_delta = (
        float(alternate_obstacle - anchor_obstacle)
        if math.isfinite(anchor_obstacle) and math.isfinite(alternate_obstacle)
        else 0.0
    )
    goal_delta = (
        float(anchor_goal - alternate_goal)
        if math.isfinite(anchor_goal) and math.isfinite(alternate_goal)
        else -math.inf
    )
    safety_admissible = (
        pair_delta >= -safety_tolerance
        and obstacle_delta >= -safety_tolerance
    )

    if hazard:
        safety_improves = max(pair_delta, obstacle_delta) >= safety_advantage_min
        goal_is_admissible = goal_delta >= -goal_loss_tolerance
        if safety_admissible and safety_improves and goal_is_admissible:
            gate_context["decision_top1_risk_active"] = True
            return 1.0, "decision_top1_risk"
        gate_context["decision_top1_risk_active"] = False
        return 0.0, "decision_top1_risk_veto"

    gate_context["decision_top1_risk_active"] = False
    if safety_admissible and goal_delta >= progress_advantage_min:
        gate_context["decision_top1_recovery_remaining"] = recovery_dwell - 1
        return 1.0, "decision_top1_recovery_trigger"
    return 0.0, "decision_top1_recovery_veto"
...
def learned_graph_gate_weights_from_features(
    mode: str,
    features: Dict[str, np.ndarray],
    learned_gate: Dict[str, object],
    gate_context: Dict[str, object],
) -> tuple[np.ndarray, str]:
    """Evaluate the learned gate and runtime floor on precomputed graph features."""
    ...
    if "adaptivefloor" in mode:
        raise ValueError(
            "Precomputed learned-graph routing does not support adaptivefloor "
            "because that floor depends on both expert actions."
        )
    if "shielded" in mode or "hardfloor" in mode or "withfloor" in mode:
        ...
        return (
            weights,
            (
                f"learned_graph_gate_floor_w{float(np.mean(weights)):.2f}"
                f"_f{float(np.mean(floor)):.2f}"
            ),
        )
...
def ensemble_weight(
    ...
    Policy A is the configured anchor and policy B is the configured alternate
    controller. The gate families are named according to the routing mechanism,
    not as a new algorithm claim.
    """
    ...
    if mode.startswith("decision_point_recovery_"):
        params = parse_keyed_floats(
            mode,
            ["w", "p", "s", "o", "h", "g", "r", "d", "c"],
        )
        return decision_point_recovery_weight(
            env,
            metrics,
            action_a,
            action_b,
    ...
    if mode.startswith("decision_point_top1_"):
        params = parse_keyed_floats(
            mode,
            ["t", "u", "o", "h", "a", "l", "r", "w", "p", "s", "g", "d", "c"],
        )
        return decision_point_top1_weight(
            env,
            metrics,
            action_a,
            action_b,
    ...
    if mode.startswith("kinematic_top1_"):
        params = parse_keyed_floats(
            mode,
            ["t", "u", "e", "q", "v", "h", "d"],
        )
        return kinematic_top1_weight(env, gate_context, params)
    ...
    if mode.startswith("graph_safety_floor_only"):
        params = parse_keyed_floats(
            mode,
            [
                "r",
        ...
        features = augment_graph_feature_dict(features, gate_context)
        floor = learned_gate_safety_floor(features, params)
        return floor, f"graph_safety_floor_only_f{float(np.mean(floor)):.2f}"
    ...
    if mode.startswith("learned_graph_gate"):
        ...
        return learned_graph_gate_weights_from_features(
            mode,
            features,
            learned_gate,
            gate_context,
